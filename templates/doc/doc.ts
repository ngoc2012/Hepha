import { $, ready, onClick } from "./selector";
...
function setModeIcon(theme: string): void {
  const pathElement = $("#app button[name=mode-toggler] path")[0] as SVGPathElement;
  if (theme === 'dark') {
...
function getBottom(e: HTMLElement): number {
...
ready(() => {
  ...
  const pages = $(".page") as NodeListOf<HTMLElement>;
  let iP = 0;
  const elements = $(".page .print") as NodeListOf<HTMLElement>;
  ...
  elements.forEach((e, iE) => {
    if (getBottom(e) > getBottom(pages[iP])) {
      if (iP >= (pages.length - 1)) {
        ...
        const newPages = $(".page") as NodeListOf<HTMLElement>;
