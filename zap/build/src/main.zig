...
fn get_extension(t: []const u8) []const u8 {
    if (std.mem.eql(u8, t, "Python")) {
        return "py";
    } else if (std.mem.eql(u8, t, "Javascript")) {
        return "js";
    } else if (std.mem.eql(u8, t, "Ruby")) {
        return "rb";
    } else if (std.mem.eql(u8, t, "Java")) {
        return "java";
    } else if (std.mem.eql(u8, t, "C")) {
        return "c";
    } else if (std.mem.eql(u8, t, "Cpp")) {
        return "cpp";
    } else if (std.mem.eql(u8, t, "Go")) {
        return "go";
    } else if (std.mem.eql(u8, t, "Rust")) {
        return "rs";
    } else if (std.mem.eql(u8, t, "Swift")) {
        return "swift";
    } else if (std.mem.eql(u8, t, "Kotlin")) {
        return "kt";
    } else if (std.mem.eql(u8, t, "Php")) {
        return "php";
    } else if (std.mem.eql(u8, t, "Perl")) {
        return "pl";
    } else if (std.mem.eql(u8, t, "R")) {
        return "r";
    } else if (std.mem.eql(u8, t, "Bash")) {
        return "sh";
    } else if (std.mem.eql(u8, t, "Powershell")) {
        return "ps1";
    } else if (std.mem.eql(u8, t, "Typescript")) {
        return "ts";
    } else if (std.mem.eql(u8, t, "Html")) {
        return "html";
    } else if (std.mem.eql(u8, t, "Css")) {
        return "css";
    } else if (std.mem.eql(u8, t, "Sql")) {
        return "sql";
    } else if (std.mem.eql(u8, t, "Json")) {
        return "json";
    } else if (std.mem.eql(u8, t, "Xml")) {
        return "xml";
    } else if (std.mem.eql(u8, t, "Yaml")) {
        return "yaml";
    } else if (std.mem.eql(u8, t, "Toml")) {
        return "toml";
    } else if (std.mem.eql(u8, t, "Ini")) {
        return "ini";
    } else if (std.mem.eql(u8, t, "Dockerfile")) {
        return "dockerfile";
    } else if (std.mem.eql(u8, t, "Plaintext")) {
        return "txt";
    } else {
        return "txt";
    }
...
fn on_request(r: zap.Request) void {
    ...
    if (r.body) |body| {
        ...
        var parsed = parseFromSlice(Value, allocator, body, .{}) catch |err| {
            r.setContentType(.TEXT) catch return;
            const message = std.fmt.allocPrint(allocator, "Error: {}!", .{err}) catch return;
            r.sendBody(message) catch return;
            return;
        };
        defer parsed.deinit();
        ...
        const box_id_node = root.object.get("id").?;
        const box_id: i64 = box_id_node.integer;
        std.debug.print("Id: {d}\n", .{box_id});

        const lang = root.object.get("lang").?;
        const extension = get_extension(lang.string);

        var code_buf = [_]u8{undefined} ** FILE_NAME_LEN;
        const code_fn = std.fmt.bufPrint(&code_buf, "{s}code_{d}.{s}", .{ STORAGE, box_id, extension }) catch return;
        var out_buf = [_]u8{undefined} ** FILE_NAME_LEN;
        const out_fn = std.fmt.bufPrint(&out_buf, "{s}output_{d}", .{ STORAGE, box_id }) catch return;
        var err_buffer = [_]u8{undefined} ** FILE_NAME_LEN;
        const err_fn = std.fmt.bufPrint(&err_buffer, "{s}error_{d}", .{ STORAGE, box_id }) catch return;

        // var dir = std.fs.openDirAbsolute(STORAGE, .{}) catch return;
        // defer dir.close();
        std.debug.print("Code file: {s}\n", .{code_fn});
        var code_file = std.fs.createFileAbsolute(code_fn, .{}) catch return;
        ...
        if (pid == 0) {
            ...
            // STDOUT
            var out_fd = std.fs.createFileAbsolute(out_fn, .{}) catch return;
            defer out_fd.close();
            ...
            // STDERR
            var fd_err = std.fs.createFileAbsolute(err_fn, .{}) catch return;
            defer fd_err.close();
            ...
            const argv = [_][]const u8{ "python3", code_fn };
            ...
            var out_fd = std.fs.openFileAbsolute(out_fn, .{}) catch return;
            defer out_fd.close();
            ...
            var err_fd = std.fs.openFileAbsolute(err_fn, .{}) catch return;
            defer err_fd.close();
